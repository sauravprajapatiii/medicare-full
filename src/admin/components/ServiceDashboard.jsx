...
import { serviceDashboardStyles as ss } from "../dummyStyles";
import {
  BadgeIndianRupee,
  Calendar,
  CheckCircle,
...
const ServiceDashboard = ({ services: servicesProp = null }) => {
  ...
  return (
    <div className={ss.container}>
      <div className={ss.innerContainer}>
        <div className={ss.header.container}>
          <div>
            <h1 className={ss.header.title}>Service Dashboard</h1>
            <p className={ss.header.subtitle}>
              Overview of services, appointments and earnings
            </p>
          </div>
          {/* refresh */}
          <div className={ss.refresh.container}>
            <div className={ss.refresh.countText}>
              {loading
                ? "Loading..."
            ...
            <button
              ...
              className={ss.refresh.button(Array.isArray(servicesProp))}
              title={
                Array.isArray(servicesProp)
                  ? "Service provided by parent components"
                  : "Refresh"
              }
            >
              Refresh
        ...
        <div className={ss.search.container}>
          <div className={ss.search.inputContainer}>
        ...
        <div className={ss.table.container}>
          <div className={ss.table.headerMd}>
            <div className={ss.table.headerText}>Service</div>
            <div className={ss.table.headerText}>Appointments</div>
            <div className={ss.table.headerText}>Completed</div>
            <div className={ss.table.headerText}>Canceled</div>
            <div className={ss.table.headerText}>Earning</div>
          </div>
          <div className={ss.table.headerLg}>
            <div className="col-span-5">Service</div>
            <div className="col-span-2">Price</div>
            <div className={ss.table.headerTextLg(1)}>Appointments</div>
            <div className={ss.table.headerTextLg(1)}>Completed</div>
            <div className={ss.table.headerTextLg(1)}>Canceled</div>
            <div className="col-span-2 text-right">Earning</div>
          </div>
          <div className={ss.table.body}>
            {loading ? (
              <div className={ss.states.loading}>Loading Services...</div>
            ) : error ? (
              <div className={ss.states.error}>Error:{error}</div>
            ) : visibleServices.length === 0 ? (
              <div className={ss.states.empty}>No Service Found</div>
            ) : (
              visibleServices.map((s) => {
                const earning = s.completed * s.price;
                return (
                  <div key={s.id} className={ss.table.row}>
                    <div className={ss.table.tabletView}>
                      <div className="flex items-center gap-3">
                        <div className={ss.table.tabletImage}>
                          <img
                            src={s.image}
                            alt={s.name}
                        ...
                        <div className={ss.table.tabletTextContainer}>
                          <div className={ss.table.tabletServiceName}>
                            {s.name}
                          </div>
                          <div className={ss.table.tabletPrice}>
                            {formatCurrency(s.price)}
                          </div>
                        </div>
                      </div>

                      <div className={ss.table.tabletCell}>
                        {s.totalAppointments}
                      </div>
                      <div
                        className={`${ss.table.tabletCell} text-emerald-700`}
                      >
                        {s.completed}
                      </div>
                      <div className={`${ss.table.tabletCell} text-red-500`}>
                        {s.canceled}
                      </div>
                      <div className={`${ss.table.tabletCell} text-right`}>
                        {formatCurrency(earning)}
                      </div>
                    </div>
                    {/* desktop */}
                    <div className={ss.table.desktopView}>
                      <div className="col-span-5 flex items-center gap-4">
                        <div className={ss.table.desktopImage}>
                          <img
                            src={s.image}
                            alt={s.name}
                            className="w-full h-full object-cover"
                        ...
                        <h3 className={ss.table.desktopServiceName}>
                          {s.name}
                        </h3>
                      </div>
                      <div className={ss.table.desktopCell(2)}>
                        {formatCurrency(s.price)}
                      </div>
                      <div className={ss.table.desktopCenterCell(1)}>
                        {s.totalAppointments}
                      </div>
                      <div className={ss.table.desktopCenterCell(1)}>
                        {s.completed}
                      ...
                      <div className={`${ss.table.desktopCell(2)} text-right`}>
                        {formatCurrency(earning)}
                      </div>
                    </div>
                    {/* mobile view */}
                    <div className={ss.table.mobileView}>
                      <div className="flex items-start gap-3">
                        <div className={ss.table.mobileImage}>
                          <img
                            src={s.image}
                            alt={s.name}
                            className="w-full h-full object-cover"
                        ...
                        <div className="flex-1 min-w-0">
                          <div className={ss.table.mobileServiceHeader}>
                            <h3 className={ss.table.mobileServiceName}>
                              {s.name}
                            </h3>
                          ...
                          <div className={ss.table.mobileStatsContainer}>
                            <div className={ss.table.mobileStatItem("emerald")}>
                              <Calendar size={14} />
                            ...
                            <div className={ss.table.mobileStatItem("red")}>
                              <XCircle size={14} />
        ...
        {filteredServices.length > INITIAL_COUNT && (
          <div className={ss.showMore.container}>
            <button
              onClick={() => setShowAll((s) => !s)}
              className={ss.showMore.button}
            >
              {showAll
                ? "Show less"
...
function StatCard({ icon, label, value }) {
  return (
    <div className={ss.statCard.container}>
      <div className={ss.statCard.iconContainer}>{icon}</div>
      <div>
        <div className={ss.statCard.label}>{label}</div>
        <div className={ss.statCard.value}>{value}</div>
      </div>
    </div>
  );
}
